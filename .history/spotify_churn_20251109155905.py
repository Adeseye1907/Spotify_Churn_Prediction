...
# --- Configuration ---
MODEL_FILE_PATH = 'churn_model.joblib' 
# CRITICAL: This list of feature names MUST be in the EXACT order your trained model expects.
# Coefficient and CCRI are explicitly placed first, as requested.
FEATURE_NAMES = [
    ...
    # One-Hot Encoded Gender (assuming 'Male' was dropped/not included as the reference category)
    'gender_Female', 'gender_Other', 
    
    # One-Hot Encoded Subscription Type (assuming 'Free' was dropped)
    'subscription_type_Family', 'subscription_type_Premium', 'subscription_type_Student',
    
    # One-Hot Encoded Device Type (assuming 'Desktop' was dropped)
    'device_type_Mobile', 'device_type_Web',
    
    # One-Hot Encoded Country features (all 8 countries, check your model for which to drop)
...
def main():
    ...
    if st.button('Predict Churn Likelihood'):
        ...
        # 3. Create the final feature dictionary (all expected features set to 0 initially)
        final_features = {name: 0 for name in FEATURE_NAMES}

        # Add INNOVATION and original numerical/binary features
        # NOTE: If you used a Scaler (e.g., StandardScaler), you must load and apply it here!
        ...
        # Add the encoded categorical features (where the value is 1)
        for col in encoded_df.columns:
            if col in final_features and encoded_df[col].iloc[0] == True:
                final_features[col] = 1
        
        # 4. Create the final DataFrame for prediction, ensuring correct order using FEATURE_NAMES
